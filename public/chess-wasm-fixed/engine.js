...
/**
 * 현재 포지션의 상세 평가(Mobility, King Safety 등)를 가져옵니다.
 * Stockfish의 'eval' 명령어를 사용합니다.
 */
async function getDetailedEval(fen) {
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      if (workerObj.busy) {
        workerObj.busy = false;
        if (isTemp) { try { worker.terminate(); } catch(err) {} }
        resolve(null);
      }
    }, 3000);
  });
}
window.getDetailedEval = getDetailedEval;

/**
 * Stockfish 'eval' 출력 테이블을 파싱하여 객체로 반환합니다.
 */
function parseEvalTable(text) {
  const result = {
    ...
    imbalance: 0,
    total: 0,
    isNNUE: text.includes('NNUE')
  };

  try {
    const lines = text.split('\n');
    for (const line of lines) {
      // Classical Eval 항목 추출 (NNUE가 아닌 경우나 혼합 모드인 경우)
      if (line.includes('Mobility')) {
        const m = line.match(/\|[^|]+\|[^|]+\|\s*(-?\d+\.\d+)/);
        if (m) result.mobility = parseFloat(m[1]);
      }
      if (line.includes('King safety')) {
        const m = line.match(/\|[^|]+\|[^|]+\|\s*(-?\d+\.\d+)/);
        if (m) result.kingSafety = parseFloat(m[1]);
      }
      if (line.includes('Space')) {
        const m = line.match(/\|[^|]+\|[^|]+\|\s*(-?\d+\.\d+)/);
        if (m) result.space = parseFloat(m[1]);
      }
      if (line.includes('Pawns')) {
        const m = line.match(/\|[^|]+\|[^|]+\|\s*(-?\d+\.\d+)/);
        if (m) result.pawnStructure = parseFloat(m[1]);
      }
      if (line.includes('Imbalance')) {
        const m = line.match(/\|[^|]+\|[^|]+\|\s*(-?\d+\.\d+)/);
        if (m) result.imbalance = parseFloat(m[1]);
      }
      if (line.includes('Final evaluation')) {
        const m = line.match(/Final evaluation\s+([+-]?\d+\.\d+)/);
        if (m) result.total = parseFloat(m[1]);
      }
    }
  } catch (e) {
    console.warn('[Engine] Eval 파싱 오류:', e);
  }
  return result;
}
