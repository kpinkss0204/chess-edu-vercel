...
function _startDragging(r, c, e) {
  if (state.puzzleSolved || state.waitingEngine || state.wrongMove) return;
  const piece = state.board[r][c];
  if (!piece) return;

  const chess = state._legalChess || new Chess(state.currentPuzzle.fen);
  if (chess.turn() !== (piece === piece.toUpperCase() ? 'w' : 'b')) return;
  ...
  _isDragging = true;
  handleSquareClick(r, c); // Select starting square
  ...
  const onUp = (ev) => {
    ...
    if (x >= 0 && x < bRect.width && y >= 0 && y < bRect.height) {
      ...
      if (state.flipped) { tc = 7 - tc; tr = 7 - tr; }
      ...
      handleSquareClick(r, c); // Deselect
    }
  };
...
(function() {
  ...
  function ensureSvg() {
    ...
    let svg = document.getElementById('board-svg-overlay') || document.getElementById('user-arrow-svg');
    if (!svg) {
    ...
    let g = document.getElementById('user-arrow-svg-arrows');
    if (!g) {
  ...
  function getBoardSquare(e) {
    ...
    if (state.flipped) { col = 7 - col; row = 7 - row; }
    return { col, row };
  }
